안녕하세요, 저는 RN팀 리더 입니다.
백엔드 팀과 함께 확인해야 할 내용이 있어서 코드를 첨부합니다. 확인바랍니다.
workplaceService.ts에 백엔드와 다른 내용이 있거나 추가되어야 하는 내용이 있다면 알려주세요.
아래 코드 뿐만 아니라 RN에서 현재 추가로 더 개발해야할 사안이 있다면 회신바랍니다.(.md)
만약, RN의 프로젝트 내용이 궁금하시다면 제가 이해하기 쉽게 프롬프트로 만들어서 .md로 생성해주세요.
